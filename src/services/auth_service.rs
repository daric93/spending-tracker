    ...
    async fn register(&self, request: CreateUserRequest) -> Result<User, AuthError> {
        ...
        let user = self.user_repository
            ...
            .map_err(|e| match e {
                ...
                RepositoryError::NotFound => AuthError::DatabaseError("Unexpected error".to_string()),
            })?;
    ...
    impl UserRepository for MockUserRepository {
        async fn create(&self, user: CreateUserRequest, password_hash: String) -> Result<User, RepositoryError> {
            ...
            if users.contains_key(&user.email) {
                return Err(RepositoryError::ConstraintViolation("Email already exists".to_string()));
            }
